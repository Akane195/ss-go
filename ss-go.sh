...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Akane195/ss-go/master/ss_go_centos" -O /etc/init.d/ss-go; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Akane195/ss-go/master/ss_go_debian" -O /etc/init.d/ss-go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "http://src.natsu.ooo:1080/ss-go/ss-go.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ss-go" ]]; then
		rm -rf /etc/init.d/ss-go
		Service
	fi
	wget -N --no-check-certificate "http://src.natsu.ooo:1080/ss-go/ss-go.sh" && chmod +x ss-go.sh
